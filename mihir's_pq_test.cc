// Copyright 2019 Mihir Wadekar and Anjali Nagaraj
...
TEST(DoubleIndexMinPQ, SimpleScenario) {
...
TEST(DoubleIndexMinPQ, ComplexScenario) {
  // Indexed min-priority queue of capacity 100
  IndexMinPQ<double> impq(100);
...
TEST(DoubleIndexMinPQ, SimpleTop) {
  // Indexed min-priority queue of capacity 100
  IndexMinPQ<double> impq(100);
  ...
  // Key-value at the top should now be (51.0, 54)
  EXPECT_EQ(impq.Top(), 54);
  impq.Pop();

  // Key-value at the top should now be (74.32, 93)
  EXPECT_EQ(impq.Top(), 93);
  impq.Pop();
...
TEST(DoubleIndexMinPQ, TopException) {
  // Indexed min-priority queue of capacity 100
  IndexMinPQ<double> impq(100);
...
TEST(DoubleIndexMinPQ, SimplePush) {
  // Indexed min-priority queue of capacity 100
  IndexMinPQ<double> impq(100);
...
TEST(DoubleIndexMinPQ, PushOverflowException) {
  // Indexed min-priority queue of capacity 100
  IndexMinPQ<double> impq(10);
  ...
  EXPECT_THROW(impq.Push(1.0, 0), std::exception);
}
